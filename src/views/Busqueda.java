...
public class Busqueda extends JFrame {
		...
		btnEliminar.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				...
				if (filaReservas >= 0) {
					...
					int confirmar = JOptionPane.showConfirmDialog(null, "¿Esta seguro de borrar esta reserva?");
					...
					int confirmar = JOptionPane.showConfirmDialog(null, "¿Esta seguro de borrar este Huesped?");
					if (confirmar == JOptionPane.YES_OPTION) {
					...
					JOptionPane.showMessageDialog(null, "¡Huebo un error al eliminar!", "Error al eliminar", JOptionPane.ERROR_MESSAGE);
				}
			}
		});
	...
	private void ActualizarReservas() {
		...
		.ifPresent(fila -> {
			...
			if(tbReservas.getSelectedColumn() == 0) {
				...
				this.reservasControl.actualizarReserva(fechaEntrada, fechaSalida, valor, formaPago, id);
	...
	private void mostrarTablaHuespedesId() {
		...
		try {
			for (Huespedes huespedes1 : huespedes) {
				modeloHuesped.addRow(new Object[] {
						huespedes1.getId(), huespedes1.getNombre(),
						huespedes1.getApellido(), huespedes1.getFechaNacimiento(),
	...
	private void actualizarHuespedes() {
		...
		.ifPresentOrElse(filaHuespedes -> {
			...
			try {
				DateTimeFormatter dateFormat = DateTimeFormatter.ofPattern("yyyy-MM-dd");
				fechaNacimiento = LocalDate.parse(modeloHuesped.getValueAt(
						tbHuespedes.getSelectedRow(), 3).toString(), dateFormat);
			} catch (DateTimeException e) {
				throw new RuntimeException(e);
			}
			//this.reservaVista.limpiarValor();
			...
			if(tbHuespedes.getSelectedColumn() == 0 || tbHuespedes.getSelectedColumn() == 6) {
				...
				this.huespedesControl.actualizarH(nombre, apellido, fechaNacimiento, nacionalidad, telefono, idReserva, id);
				JOptionPane.showMessageDialog(this, String.format("¡Registro modificado con éxito!"));
			}
			
		}, ()-> JOptionPane.showInputDialog(this, "Por favor no seas animal Xd"));	
	}
